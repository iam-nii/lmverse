...
export default function Teachers() {
  ...
  if (loading)
    return <div className="p-8 text-gray-500">Loading tutors...</div>;
      ...
      <header className="flex items-center px-8 py-4 bg-white border-b border-gray-100 shadow-sm">
        <div className="flex items-center gap-2 text-sm text-gray-500 font-medium">
          <button
            ...
            className="hover:text-[#16476a] transition-colors"
          >
            Teachers
          </button>
          <ChevronRight size={16} className="text-gray-300" />
          <span className="text-gray-900 font-semibold">
            {selectedTutor.full_name || "Tutor Details"}
          </span>
        </div>
      </header>
      ...
      <main className="p-8 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 bg-white p-6 rounded-[1rem] shadow-sm border border-gray-100">
            ...
            <div className="mt-1">
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2 tracking-tight">
                ...
                {selectedTutor.is_approved && (
                  <Check
                    className="text-white bg-[#409a38] rounded-full p-0.5"
                    size={18}
                  />
                )}
              </h1>
          ...
          <div className="flex items-center gap-3 bg-[#f8e5fe]/50 px-5 py-3.5 rounded-[1rem] border border-[#e8ccf1] shadow-sm">
            <span className="text-sm font-bold tracking-tight text-[#6e2a8d]">
              Activate teacher account
            </span>
            <button
              onClick={() => handleToggleActivation(selectedTutor)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#6e2a8d]/30 focus:ring-offset-2 ${
                selectedTutor.is_approved ? "bg-[#16476a]" : "bg-gray-300"
              }`}
            >
              <span
            ...
            <div
              className="w-[20px] h-[20px] rounded-full border border-[#6e2a8d]/50 flex items-center justify-center text-[#6e2a8d] font-bold text-[10px] ml-1 cursor-help hover:bg-[#6e2a8d]/10 transition-colors"
              title="Toggle to approve or suspend the tutor"
            >
              ?
            </div>
          </div>
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white p-6 rounded-[1rem] shadow-sm border border-gray-100 h-full">
              ...
              <p className="text-gray-600 leading-relaxed text-sm whitespace-pre-wrap">
                {selectedTutor.about ||
                  "This tutor hasn't provided a biography yet."}
              </p>
            </div>
          </div>
              ...
              <ul className="space-y-4 text-sm">
                <li className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-gray-900">
                    {selectedTutor.phone_number || "N/A"}
                  </span>
                </li>
                <li className="flex justify-between items-center">
