...
import { Mail, EyeOff, Eye, Check } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white text-slate-800 p-4">
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="flex justify-between items-center mb-10">
          <div className="flex items-center space-x-2">
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-green-400 flex items-center justify-center">
              <span className="text-white font-bold text-xs tracking-tighter">LM</span>
            </div>
            <span className="text-xl font-bold text-slate-900 tracking-tight">Lmverse</span>
          </div>
        ...
        {/* Title */}
        <div>
          <h2 className="text-3xl font-extrabold text-slate-900 mb-6">Sign into Your Account</h2>
        </div>
        ...
        <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex items-center justify-between pt-1">
            <label className="flex items-center space-x-2 cursor-pointer group">
              <div
                className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${rememberMe ? 'bg-emerald-600 border-emerald-600' : 'border-slate-300 group-hover:border-emerald-500'}`}
        ...
        <div className="text-center mt-6">
          <span className="text-sm text-slate-500">Don't you have an account? </span>
          <Link href="/auth/signup" className="text-sm font-medium text-emerald-600 hover:text-emerald-500 hover:underline">
            Sign up
          </Link>
        </div>
