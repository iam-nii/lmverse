...
export function MenuCloseIcon({ size = 40, color = "currentColor", className, duration = 2000, isOpen }: StateIconProps & { isOpen?: boolean }) {
    // Use passed state if available, otherwise fallback to auto-toggle
    const autoOpen = useAutoToggle(duration);
    const open = isOpen !== undefined ? isOpen : autoOpen;
...
export function EyeToggleIcon({ size = 40, color = "currentColor", className, duration = 2200, isHidden }: StateIconProps & { isHidden?: boolean }) {
    const autoHidden = useAutoToggle(duration);
    const hidden = isHidden !== undefined ? isHidden : autoHidden;
